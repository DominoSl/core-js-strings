...
function removeFirstOccurrences(str, value) {
  ...
  return str.slice(0, idx).concat(str.slice(idx + value.length));
}
...
function removeLastOccurrences(str, value) {
  const idx = str.lastIndexOf(value);
...
function sumOfCodes(str = '') {
  return str ? str.split('').reduce((acc, ch) => acc + ch.charCodeAt(0), 0) : 0;
}
...
function formatTime(minutes, seconds) {
  ...
  return `${stringM.padStart(2, '0')}:${stringS.padStart(2, '0')}`;
}
...
function countVowels(str) {
  ...
  return lowerStr.split(/[aeoiyu]/).length - 1;
}
...
function isPalindrome(str) {
  const clean = str.replace(/[ ,!?-]/g, '').toLocaleLowerCase();
...
function encodeToRot13(str) {
  ...
  const arrayOfChars = str
    .split('')
    .map((ch) => (alph.includes(ch) ? alph[alph.indexOf(ch) + 13] : ch));
